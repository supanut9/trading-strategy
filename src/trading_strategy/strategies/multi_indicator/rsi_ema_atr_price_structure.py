...
class RsiEmaAtrPriceStructureStrategy(Strategy):
    period: int
    oversold: float
    overbought: float
    ema_window: int
    ...
    name: str = "rsi_ema_atr_price_structure"
    ...
    def desired_position(self, index: int, candles: list[Candle]) -> int:
        ...
        if self.ema_window <= 0 or self.ema_slope_window <= 0 or self.atr_period <= 0 or self.structure_window <= 1:
        ...
        required_bars = max(
            self.period + 2,
            self.ema_window + self.ema_slope_window,
            self.atr_period + 1,
        ...
        recent_high = max(highs[-self.structure_window :])
        recent_low = min(lows[-self.structure_window :])
        ...
        supportive_structure = range_midpoint >= (current_ema * 0.995)

        if bullish_regime and healthy_volatility and supportive_structure and current_rsi <= self.oversold:
            return 1
