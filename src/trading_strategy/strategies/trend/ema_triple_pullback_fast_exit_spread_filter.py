...
class EmaTriplePullbackFastExitSpreadFilterStrategy(Strategy):
    fast_window: int
    middle_window: int
    slow_window: int
    pullback_pct: float
    min_fast_middle_spread_pct: float
    min_middle_slow_spread_pct: float
    name: str = "ema_triple_pullback_fast_exit_spread_filter"
    ...
    def parameters(self) -> dict:
        return {
            ...
            "min_fast_middle_spread_pct": self.min_fast_middle_spread_pct,
            "min_middle_slow_spread_pct": self.min_middle_slow_spread_pct,
        }
    ...
    def desired_position(self, index: int, candles: list[Candle]) -> int:
        ...
        if self.min_fast_middle_spread_pct < 0 or self.min_middle_slow_spread_pct < 0:
        ...
        if not (fast_ema > middle_ema > slow_ema):
            return 0
        if current_close < fast_ema:
            return 0

        fast_middle_spread_pct = (fast_ema - middle_ema) / middle_ema
        middle_slow_spread_pct = (middle_ema - slow_ema) / slow_ema
        if fast_middle_spread_pct < self.min_fast_middle_spread_pct:
            return -1
        if middle_slow_spread_pct < self.min_middle_slow_spread_pct:
            return -1
