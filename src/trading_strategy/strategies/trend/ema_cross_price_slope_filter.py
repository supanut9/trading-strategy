...
class EmaCrossPriceSlopeFilterStrategy(Strategy):
    short_window: int
    long_window: int
    filter_window: int
    slope_window: int
    name: str = "ema_cross_price_slope_filter"
    ...
    def desired_position(self, index: int, candles: list[Candle]) -> int:
        ...
        minimum_bars = self.filter_window + self.slope_window
        ...
        past_filter_ema = exponential_moving_average(closes[: -self.slope_window], self.filter_window)
        current_close = closes[-1]

        return 1 if short_ema > long_ema and current_close > filter_ema and filter_ema > past_filter_ema else 0
